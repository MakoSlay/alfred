...
import { recallProfile } from "../src/alfred-2/tools/profile.ts";
...
test("recall returns a structured tool failure when profile loading fails", () => {
	const { directory, file } = temporaryProfile();
	writeFileSync(file, "{ broken", "utf8");
	try {
		const result = recallProfile(
			{ tool: "recall" },
			{ requestId: "req-recall", toolCallId: "tool-recall", risk: "read" },
			createProfileStore(file),
		);
		assert.equal(result.success, false);
		assert.equal(result.retryable, true);
		assert.match(result.text, /existing file was left untouched/);
	} finally {
		rmSync(directory, { recursive: true, force: true });
	}
});
